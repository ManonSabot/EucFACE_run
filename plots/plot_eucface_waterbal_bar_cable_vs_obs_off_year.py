...
i = 5
print(np.sum(amb.iloc[0:4].values,axis=0))
print(np.sum(amb_obs[0:4],axis=0))
...
rects1 = ax.bar(x - 0.15, np.sum(amb_obs[0:4],axis=0), width/2, color='royalblue', label='Obs')
rects2 = ax.bar(x + 0.15, np.sum(amb.iloc[0:4].values,axis=0), width/2, color='deepskyblue', label='CABLE')
...
fig.savefig('water_balance_2013_or_off', bbox_inches='tight',pad_inches=0.1)
