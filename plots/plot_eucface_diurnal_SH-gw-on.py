...
# _________________________ CABLE ___________________________
    ...
    var = pd.DataFrame(cable.variables['Qh'][:,0,0],columns=['Qh'])
    #var['Qle'] = cable.variables['Qg'][:,0,0] + cable.variables['Qle'][:,0,0] + cable.variables['Qh'][:,0,0]
    var['Qle'] = cable.variables['Qle'][:,0,0]
    var['Qg'] = cable.variables['Qg'][:,0,0]
    ...
    print(var)
    plt.plot(var['Qle'],c="orange",label="Qle")
    plt.plot(var['Qh'],c="green",label="Qh")
    plt.plot(var['Qg'],c="blue",label="Qg")
    plt.ylim((-200,500))
    plt.legend()
    plt.show()
    '''
...
if __name__ == "__main__":
    ...
    for ring in rings:
        ...
        fcable ="/srv/ccrc/data25/z5218916/cable/EucFACE/EucFACE_run/outputs/%s/EucFACE_%s_out.nc" \
                    % (case_name[0], ring)
        main(fobs, fcable, case_name[0], ring, term)
