...
def read_cable_swc_50cm(output_file, met_case):
    ...
    print(SoilMoist['cable'])
    print(SoilMoist.index)
    """
...
def read_cable_swc_all(output_file, met_case):
...
def calc_cable_esoil2trans(output_file):
    ...
    Esoil_2_Trans   = Esoil_cable/Trans_cable
    print(Esoil_2_Trans)
...
def read_obs_esoil(ring):
    ...
    est_esoil = pd.read_csv(fobs, usecols = ['Ring','Date','wuTP','EfloorPred'])
...
def calc_obs_esoil2trans(ring):
    ...
    # unify dates
    Esoil_obs = subs_Esoil['wuTP'].loc[subs_Esoil.index.isin(subs_Trans.index)]
    Trans_obs = subs_Trans['volRing'].loc[subs_Trans.index.isin(subs_Esoil.index)]
...
if __name__ == "__main__":
    ...
    output_file = "/srv/ccrc/data25/z5218916/cable/EucFACE/EucFACE_run_opt_31uni_hyds-50cm/outputs/met_LAI_vrt_swilt-watr-ssat_SM_31uni_hyds^20_fw-hie-exp/EucFACE_amb_out.nc"
    met_case    = "met_LAI_vrt_swilt-watr-ssat_SM_31uni"
    var_name    = "TVeg"
    ring        = "amb"

    a = read_cable_swc_50cm(output_file, met_case)
    b = read_cable_swc_all(output_file, met_case)
    c = read_cable_var(output_file, var_name)
    d = calc_cable_esoil2trans(output_file)
    e = read_obs_swc_tdr(ring)
    f = read_obs_swc_neo(ring)
    g = read_obs_trans(ring)
    h = read_obs_esoil(ring)
    i = calc_obs_esoil2trans(ring)
