...
def plot_waterbal_no_total_Evap_imbalance(path, case_names, case_labels):
    ...
    for case_num in np.arange(case_sum):
        ...
        cable_amb['Qs'] = cable_amb['Qs'] # + cable_amb['Qsb'] # subsurface runoff shouldn't be in runoff
    ...
    labels = ['$P$','$E_{tr}$','$E_{s}$','$E_{c}$','$R$','$D_{r}$','$ΔS$','$Imb$']
    x = np.arange(len(labels))  # the label locations
    width = 1/(case_sum+3)                # the width of the bars
    ...
    # Add some text for labels, title and custom x-axis tick labels, etc.
    ax.set_ylabel('Component (mm y$^{-1}$)')
    #ax.set_title(title)
    ax.set_ylim(-250, 650)
    ax.set_xticks(x)
    ax.set_xticklabels(labels)
...
def plot_waterbal_no_total_Evap_error_imbalance(path, case_names, case_labels):
    ...
    case_sum = len(case_names)
    cable_year_amb = np.zeros([case_sum,8])
    cable_year_R2 = np.zeros([case_sum,8])
    cable_year_R3 = np.zeros([case_sum,8])
    cable_year_R6 = np.zeros([case_sum,8])
    cable_errors = np.zeros([2,case_sum,8])
    ...
    for case_num in np.arange(case_sum):
        ...
        cable_R2       = pd.read_csv('%sEucFACE_R2_%s.csv' % (path, case_names[case_num]),
            usecols = ['Year','Season','Rainf','TVeg','ESoil','ECanop','Qs','Qsb','Qrecharge','soil_storage_chg'])
        cable_R2['Qs'] = cable_R2['Qs'] + cable_R2['Qsb']
        cable_R2       = cable_R2.drop(['Year','Season','Qsb'], axis=1)
        cable_R2       = cable_R2.drop([0])
        cable_year_R2[case_num,:7] = np.sum(cable_R2.iloc[1:5].values,axis=0)
        #print(cable_year_R2[case_num,:])

        cable_R3       = pd.read_csv('%sEucFACE_R3_%s.csv' % (path, case_names[case_num]),
            usecols = ['Year','Season','Rainf','TVeg','ESoil','ECanop','Qs','Qsb','Qrecharge','soil_storage_chg'])
        cable_R3['Qs'] = cable_R3['Qs'] + cable_R3['Qsb']
        cable_R3       = cable_R3.drop(['Year','Season','Qsb'], axis=1)
        cable_R3       = cable_R3.drop([0])
        cable_year_R3[case_num,:7] = np.sum(cable_R3.iloc[1:5].values,axis=0)
        #print(cable_year_R3[case_num,:])

        cable_R6       = pd.read_csv('%sEucFACE_R6_%s.csv' % (path, case_names[case_num]),
            usecols = ['Year','Season','Rainf','TVeg','ESoil','ECanop','Qs','Qsb','Qrecharge','soil_storage_chg'])
        cable_R6['Qs'] = cable_R6['Qs'] + cable_R6['Qsb']
        cable_R6       = cable_R6.drop(['Year','Season','Qsb'], axis=1)
        cable_R6       = cable_R6.drop([0])
        cable_year_R6[case_num,:7] = np.sum(cable_R6.iloc[1:5].values,axis=0)
        #print(cable_year_R6[case_num,:])
        ...
        cable_year_R2[case_num,7] = cable_year_R2[case_num,0]  \
                                     - cable_year_R2[case_num,1]  \
                                     - cable_year_R2[case_num,2]  \
                                     - cable_year_R2[case_num,3]  \
                                     - cable_year_R2[case_num,4]  \
                                     - cable_year_R2[case_num,5]  \
                                     - cable_year_R2[case_num,6]

        cable_year_R3[case_num,7] = cable_year_R3[case_num,0]  \
                                     - cable_year_R3[case_num,1]  \
                                     - cable_year_R3[case_num,2]  \
                                     - cable_year_R3[case_num,3]  \
                                     - cable_year_R3[case_num,4]  \
                                     - cable_year_R3[case_num,5]  \
                                     - cable_year_R3[case_num,6]

        cable_year_R6[case_num,7] = cable_year_R6[case_num,0]  \
                                     - cable_year_R6[case_num,1]  \
                                     - cable_year_R6[case_num,2]  \
                                     - cable_year_R6[case_num,3]  \
                                     - cable_year_R6[case_num,4]  \
                                     - cable_year_R6[case_num,5]  \
                                     - cable_year_R6[case_num,6]

        cable_errors[0,case_num,:] = cable_year_amb[case_num,:] - \
                                     np.min([cable_year_R2[case_num,:],cable_year_R3[case_num,:],
                                     cable_year_R6[case_num,:]], axis = 0)
        cable_errors[1,case_num,:] = np.max([cable_year_R2[case_num,:],cable_year_R3[case_num,:],
                                     cable_year_R6[case_num,:]], axis = 0) \
                                     - cable_year_amb[case_num,:]


        print("==========================================")
        print(cable_year_amb[case_num,:])
        print(np.min([cable_year_R2[case_num,:],cable_year_R3[case_num,:],cable_year_R6[case_num,:]], axis = 0))
        print(np.max([cable_year_R2[case_num,:],cable_year_R3[case_num,:],cable_year_R6[case_num,:]], axis = 0))
    ...
    for case_num in np.arange(case_sum):
        print(case_num)
        ax.bar(x + offset + (case_num+1)*width, cable_year_amb[case_num,:], width,
               color=colors[case_num], label=case_labels[case_num]) # yerr=cable_errors[:,case_num,:],
...
def calc_waterbal(fcable, layer, ring):
    ...
    # output
    df_cable.to_csv("./csv/EucFACE_%s_%s.csv" %(ring, fcable.split("/")[-2]))
