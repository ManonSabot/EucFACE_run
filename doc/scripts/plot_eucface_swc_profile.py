...
def plot_profile_tdr_ET(fcable, ring, contour, layer):
    ...
    ax1.scatter(subs_Trans.index, subs_Trans['obs'].rolling(window=3).mean(), marker='o', c='',edgecolors='blue', s = 2., label="Trans Obs")
    ax1.scatter(subs_Esoil.index, subs_Esoil['obs'].rolling(window=3).mean(), marker='o', c='',edgecolors='red', s = 2., label="ESoil Obs")
...
def plot_profile_ET_error_rain(fpath, case_name, ring, contour, layer):
    ...
    ax2.fill_between(x, ESoil['min'].rolling(window=3).mean(),
        ESoil['max'].rolling(window=3).mean(), color="orange", alpha=0.2)
    ax2.plot(x, ESoil['ESoil'].rolling(window=3).mean(),
        c="orange", lw=1.0, ls="-", label="$E_{s}$ (CABLE)") #.rolling(window=7).mean()
    ax2.fill_between(x, ESoil['obs_min'].rolling(window=3).mean(),
        ESoil['obs_max'].rolling(window=3).mean(), color="red", alpha=0.2)
    ax2.scatter(subs_Esoil.index, subs_Esoil['obs'].rolling(window=3).mean(),
        marker='o', c='',edgecolors='red', s = 2., label="$E_{s}$ (Obs)")
    ax2.text(0.02, 0.95, '(b)', transform=ax2.transAxes, fontsize=14, verticalalignment='top', bbox=props)
    ...
    if case_name == "met_LAI_6":
        ...
        ax7  = ax2.twinx()
        ax7.set_ylabel('$P$ (mm d$^{-1}$)')
        ax7.bar(x, -Rainf['Rainf'],  1., color='gray', alpha = 0.5, label='Rainfall') # 'royalblue'
        ax7.set_ylim(-220.,0)
        ax7.set_xlim(367,1097)
        y_ticks      = [-200,-150,-100,-50,0.]
        y_ticklabels = ['200','150','100','50','0']
        ax7.set_yticks(y_ticks)
        ax7.set_yticklabels(y_ticklabels)
