...
def plot_single_HW_event(time_scale, case_labels, i, date, Tair, Rainf, Qle, Qh, EF, SM_top, SM_mid, SM_bot, SM_all, SM_15m):
    ...
    if i == 6:
        ...
        ax5.set_ylabel("$θ$$_{1.5m}$ (m$^{3}$ m$^{-3}$)") #(m$^{3}$ m$^{-3}$)")
        ax5.axis('tight')
        #ax5.legend()
...
def plot_case_study_HW_event(fcables, fcables_re, case_labels, ring, layers):
    ...
    for case_num in np.arange(len(case_labels)):
        ...
        if case_num <2 :
            ...
            ax3.plot(x, Qle[case_num, :], c=colors[case_num+4], lw=1.5, ls='-', label=case_labels[case_num])#.rolling(window=30).mean()
            ax4.plot(x, Qh[case_num, :],  c=colors[case_num+4], lw=1.5, ls='-', label=case_labels[case_num]) #, label=case_labels)#.rolling(window=30).mean()
            ...
            if case_num > 0:
                ax3.plot(x, Qle_re[case_num, :], c=colors[case_num+4], lw=1.5, ls='--')#.rolling(window=30).mean()
                ax4.plot(x, Qh_re[case_num, :],  c=colors[case_num+4], lw=1.5, ls='--') #, label=case_labels)#.rolling(window=30).mean()
