...
def plot_Trans_SM( fstd, fhvrd, fexp, fwatpot, layer, ring):

    SM_std  = read_cable_SM(fstd, layer)
    fw_std  = read_cable_var(fstd, "TVeg")
    print(SM_std)
    print(fw_std)
    sm_std = SM_std.iloc[:,0:10].mean(axis = 1)

    SM_hvrd  = read_cable_SM(fhvrd, layer)
    fw_hvrd  = read_cable_var(fhvrd, "TVeg")
    sm_hvrd = SM_hvrd.iloc[:,0:10].mean(axis = 1)

    SM_exp  = read_cable_SM(fexp, layer)
    fw_exp  = read_cable_var(fexp, "TVeg")
    sm_exp = SM_exp.iloc[:,0:10].mean(axis = 1)

    SM_watpot  = read_cable_SM(fwatpot, layer)
    fw_watpot  = read_cable_var(fwatpot, "TVeg")
    sm_watpot = SM_watpot.iloc[:,0:10].mean(axis = 1)
...
def plot_Esoil_SM( fstd, fhvrd, fexp, fwatpot, layer, ring):

    SM_std  = read_cable_SM(fstd, layer)
    fw_std  = read_cable_var(fstd, "ESoil")
    print(SM_std)
    print(fw_std)
    sm_std = SM_std.iloc[:,0:10].mean(axis = 1)

    SM_hvrd  = read_cable_SM(fhvrd, layer)
    fw_hvrd  = read_cable_var(fhvrd, "ESoil")
    sm_hvrd = SM_hvrd.iloc[:,0:10].mean(axis = 1)

    SM_exp  = read_cable_SM(fexp, layer)
    fw_exp  = read_cable_var(fexp, "ESoil")
    sm_exp = SM_exp.iloc[:,0:10].mean(axis = 1)

    SM_watpot  = read_cable_SM(fwatpot, layer)
    fw_watpot  = read_cable_var(fwatpot, "ESoil")
    sm_watpot = SM_watpot.iloc[:,0:10].mean(axis = 1)
...
if __name__ == "__main__":
    ...
    ring = "amb"
    layer= "31uni"
    case_5 = "/srv/ccrc/data25/z5218916/cable/EucFACE/EucFACE_run/outputs/met_LAI_vrt_swilt-watr-ssat_SM_31uni_hydsx10-x100-x100_litter"
    fstd ="%s/EucFACE_%s_out.nc" % (case_5, ring)

    case_6 = "/srv/ccrc/data25/z5218916/cable/EucFACE/EucFACE_run/outputs/met_LAI_vrt_swilt-watr-ssat_SM_31uni_hydsx10-x100-x100_litter_Hvrd"
    fhvrd ="%s/EucFACE_%s_out.nc" % (case_6, ring)

    case_7 = "/srv/ccrc/data25/z5218916/cable/EucFACE/EucFACE_run/outputs/met_LAI_vrt_swilt-watr-ssat_SM_31uni_hydsx10-x100-x100_litter_hie-exp"
    fexp ="%s/EucFACE_%s_out.nc" % (case_7, ring)

    case_8 = "/srv/ccrc/data25/z5218916/cable/EucFACE/EucFACE_run/outputs/met_LAI_vrt_swilt-watr-ssat_SM_31uni_hydsx10-x100-x100_litter_hie-watpot"
    fwatpot ="%s/EucFACE_%s_out.nc" % (case_8, ring)

    plot_fwsoil_SM( fstd, fhvrd, fexp, fwatpot, layer, ring)
    plot_Trans_SM( fstd, fhvrd, fexp, fwatpot, layer, ring)
    plot_Esoil_SM( fstd, fhvrd, fexp, fwatpot, layer, ring)
